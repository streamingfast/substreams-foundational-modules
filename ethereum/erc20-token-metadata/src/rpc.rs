...
pub fn batch_name<'a>(contracts: &'a [&[u8]], chunk_size: usize) -> HashMap<&'a [u8], String> {
    ...
    for chunks in contracts.chunks(chunk_size) {
        ...
        substreams::log::info!(
            "Executing erc20::functions::Name RpcBatch for {} addresses",
            chunks.len()
        );
        let responses = batch
            .execute()
            .expect("failed to execute erc20::functions::Name RpcBatch")
            .responses;
        for (i, address) in chunks.iter().enumerate() {
            ...
            >(&responses[i])
            {
                ...
                substreams::log::info!(
                    "Failed to decode erc20::functions::Name for address={:?}",
                    Hex::encode(address)
                );
            }
            ...
            >(&responses[i])
            {
                ...
                substreams::log::info!(
                    "Failed to decode sai::functions::Name for address={:?}",
                    Hex::encode(address)
                );
            }
        }
...
pub fn batch_symbol<'a>(contracts: &'a [&[u8]], chunk_size: usize) -> HashMap<&'a [u8], String> {
    ...
    for chunks in contracts.chunks(chunk_size) {
        ...
        let responses = batch
            .execute()
            .expect("failed to execute erc20::functions::Symbol RpcBatch")
            .responses;
        for (i, address) in chunks.iter().enumerate() {
            // erc20::Symbol
            ...
            >(&responses[i])
            {
                ...
                substreams::log::info!(
                    "Failed to decode erc20::functions::Symbol for address={:?}",
                    Hex::encode(address)
                );
            }
            ...
            >(&responses[i])
            {
                ...
                substreams::log::info!(
                    "Failed to decode sai::functions::Symbol for address={:?}",
                    Hex::encode(address)
                );
            }
        }
...
pub fn batch_decimals<'a>(contracts: &'a [&[u8]], chunk_size: usize) -> HashMap<&'a [u8], i32> {
    ...
    for chunks in contracts.chunks(chunk_size) {
        ...
        let responses = batch
            .execute()
            .expect("failed to execute erc20::functions::Decimals RpcBatch")
            .responses;
        for (i, address) in chunks.iter().enumerate() {
            // erc20::Decimals
            ...
            >(&responses[i])
            {
                if let Some(decimals_u8) = bigint_to_uint8(&decimals) {
                    ...
                    substreams::log::info!(
                        "Decimals out of range for address={:?}",
                        Hex::encode(address)
                    );
                }
            } else {
                substreams::log::info!(
                    "Failed to decode erc20::functions::Decimals for address={:?}",
                    Hex::encode(address)
                );
            }
        }
