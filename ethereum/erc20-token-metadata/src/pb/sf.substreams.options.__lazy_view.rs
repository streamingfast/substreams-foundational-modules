...
/** Lazy view of `sf.substreams.FieldOptions`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `FieldOptionsView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `FieldOptionsView`-family
 ...
 let view = FieldOptionsLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct FieldOptionsLazyView<'a> {
...
impl<'a> ::buffa::LazyMessageView<'a> for FieldOptionsLazyView<'a> {
...
impl<'a> ::buffa::MessageName for FieldOptionsLazyView<'a> {
