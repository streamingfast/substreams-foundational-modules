...
/** Lazy view of `sf.substreams.ethereum.erc20.v1.TokenMetadata`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `TokenMetadataView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `TokenMetadataView`-family
 ...
 let view = TokenMetadataLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct TokenMetadataLazyView<'a> {
    /// Field 1: `address`
    pub address: &'a [u8],
    /// Field 2: `name`
    pub name: &'a str,
...
impl<'a> TokenMetadataLazyView<'a> {
...
impl<'a> ::buffa::LazyMessageView<'a> for TokenMetadataLazyView<'a> {
...
impl<'a> ::buffa::MessageName for TokenMetadataLazyView<'a> {
