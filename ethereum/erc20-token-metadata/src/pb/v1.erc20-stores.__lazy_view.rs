...
/** Lazy view of `evm.erc20.stores.v1.Events`: nested and repeated message fields are
...
pub struct EventsLazyView<'a> {
    ...
    pub first_transfer_by_contract: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::FirstTransferByContractLazyView<'a>,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                1u32 => {
                    ...
                    view.first_transfer_by_contract.push_bytes(sub, __sub_ctx);
                }
                _ => {
        ...
        ::core::result::Result::Ok(super::super::Events {
            first_transfer_by_contract: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(
                    self.first_transfer_by_contract.len(),
                );
                for __r in self.first_transfer_by_contract.iter() {
...
impl<'a> EventsLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        let mut size = 0u64;
        for __frag in self.first_transfer_by_contract.raw_elements() {
            size
...
/** Lazy view of `evm.erc20.stores.v1.FirstTransferByContract`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `FirstTransferByContractView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `FirstTransferByContractView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = FirstTransferByContractLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct FirstTransferByContractLazyView<'a> {
    /// Field 1: `contract`
    pub contract: &'a [u8],
}
impl<'a> FirstTransferByContractLazyView<'a> {
    /// Decode from `buf` under the limits carried by `ctx`, recording
    /// nested/repeated message fields as byte ranges.
    ///
    /// **Not part of the public API.**
...
impl<'a> ::buffa::LazyMessageView<'a> for FirstTransferByContractLazyView<'a> {
...
impl<'a> ::buffa::MessageName for FirstTransferByContractLazyView<'a> {
    const PACKAGE: &'static str = "evm.erc20.stores.v1";
