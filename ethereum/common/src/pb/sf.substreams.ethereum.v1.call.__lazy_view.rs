...
/** Lazy view of `sf.substreams.ethereum.v1.Calls`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `CallsView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `CallsView`-family
 ...
 let view = CallsLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct CallsLazyView<'a> {
    /// Field 1: `clock`
...
impl<'a> ::buffa::LazyMessageView<'a> for CallsLazyView<'a> {
...
impl<'a> ::buffa::MessageName for CallsLazyView<'a> {
...
/** Lazy view of `sf.substreams.ethereum.v1.Call`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
