...
/** Lazy view of `sf.substreams.ethereum.v1.EventsAndCalls`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `EventsAndCallsView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `EventsAndCallsView`-family
 ...
 let view = EventsAndCallsLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct EventsAndCallsLazyView<'a> {
    /// Field 1: `clock`
...
impl<'a> EventsAndCallsLazyView<'a> {
...
impl<'a> ::buffa::LazyMessageView<'a> for EventsAndCallsLazyView<'a> {
...
impl<'a> ::buffa::MessageName for EventsAndCallsLazyView<'a> {
