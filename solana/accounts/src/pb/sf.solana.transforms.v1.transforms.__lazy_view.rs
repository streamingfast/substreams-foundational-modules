...
/** Lazy view of `sf.solana.transforms.v1.ProgramFilter`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `ProgramFilterView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `ProgramFilterView`-family
 ...
 let view = ProgramFilterLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct ProgramFilterLazyView<'a> {
    /// Field 1: `program_ids`
    pub program_ids: ::buffa::RepeatedView<'a, &'a str>,
}
impl<'a> ProgramFilterLazyView<'a> {
...
impl<'a> ::buffa::LazyMessageView<'a> for ProgramFilterLazyView<'a> {
...
impl<'a> ::buffa::MessageName for ProgramFilterLazyView<'a> {
