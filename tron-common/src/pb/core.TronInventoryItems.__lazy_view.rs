...
/** Lazy view of `protocol.InventoryItems`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `InventoryItemsView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `InventoryItemsView`-family
 ...
 let view = InventoryItemsLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct InventoryItemsLazyView<'a> {
    /// Field 1: `type`
    pub r#type: i32,
    /// Field 2: `items`
    pub items: ::buffa::RepeatedView<'a, &'a [u8]>,
}
impl<'a> InventoryItemsLazyView<'a> {
...
impl<'a> ::buffa::LazyMessageView<'a> for InventoryItemsLazyView<'a> {
...
impl<'a> ::buffa::MessageName for InventoryItemsLazyView<'a> {
