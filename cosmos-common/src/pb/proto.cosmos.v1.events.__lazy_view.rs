...
/** Lazy view of `sf.substreams.cosmos.v1.EventList`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `EventListView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `EventListView`-family
 ...
 let view = EventListLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct EventListLazyView<'a> {
    /// Field 1: `clock`
...
impl<'a> ::buffa::LazyMessageView<'a> for EventListLazyView<'a> {
...
impl<'a> ::buffa::MessageName for EventListLazyView<'a> {
...
/** Lazy view of `sf.substreams.cosmos.v1.Event`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
...
pub struct EventLazyView<'a> {
    ...
    /// Field 2: `event`
    pub event: ::buffa::LazyMessageFieldView<
        'a,
        super::super::super::super::super::cosmos::r#type::v2::__buffa::lazy_view::EventLazyView<
            'a,
        >,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                ...
                2u32 => {
                    ...
                    let sub = ::buffa::types::borrow_bytes(&mut cur)?;
                    ctx.register_element_memory(::core::mem::size_of::<&'a [u8]>())?;
                    view.event.push_fragment(sub, __sub_ctx);
                }
                _ => {
                    ::buffa::encoding::skip_field_depth(tag, &mut cur, ctx.depth())?;
                }
...
impl<'a> EventLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        for __frag in self.event.fragments() {
            size
                += 1u64 + ::buffa::encoding::varint_len(__frag.len() as u64) as u64
                    + __frag.len() as u64;
        }
