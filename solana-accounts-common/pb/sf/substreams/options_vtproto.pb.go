...
func (m *FieldOptions) CloneVT() *FieldOptions {
	...
	r := new(FieldOptions)
	r.LoadFromFile = m.LoadFromFile
	r.ZipFromFolder = m.ZipFromFolder
...
func (this *FieldOptions) EqualVT(that *FieldOptions) bool {
	...
	if this.LoadFromFile != that.LoadFromFile {
		return false
	}
	if this.ZipFromFolder != that.ZipFromFolder {
		return false
	}
...
func (m *FieldOptions) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field LoadFromFile", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.LoadFromFile = bool(v != 0)
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ZipFromFolder", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
